...
%Roundabout Map = map2:
pointLocations2 = [5 40; 30 40; 30 25 ;70 40;70 25;5 60;30 60;30 75;70 60;70 75;85 40];
connections2{1} = [2 ; 25];
connections2{2} = [3 4 ; 15 40];
connections2{3} = [4 ;50];
connections2{4} = [ 5 9; 15 35];
connections2{5} = [11 ; 50];
connections2{6} = [1 ;100 ];
connections2{7} = [2 6 8 ;35 25 15];
connections2{8} = [6;50];
connections2{9} = [7 10; 40 15];
connections2{10} =[7;50];
connections2{11} =[9;50];
...
%dijkstra(robotState1.x,robotState1.y,8);%[0.25 2;1.5 2;1.5 3;1.5 3.75];
%dijkstra(robotState2.x,robotState2.y,3);%;[0.25 3;1.5 3;1.5 2;1.5 1.25];
path1 = [1 2 7 8];
path2 = [6 7 2 4 5];%[6 7 2 4 5];
path1 = chk_pt(path1,:);
path2 = chk_pt(path2,:);
...
while (i<=size(path1,1) || j<=size(path2,1)) && (~EndFlag1 || ~EndFlag2)
    ...
    while robotState1.heading ~= atan2(path1(i,2)-robotState1.y,abs(path1(i,1)-robotState1.x))
        omega = sign(atan2(path1(i,2)-robotState1.y,abs(robotState1.x-path1(i,1))) - robotState1.heading ) * deg2rad(60);
...
%     end
    ...
    if isequal(path1(end,:),[robotState1.x robotState1.y])
        EndFlag1 = true;
        %disp('Goal 1 Reached.')
    end
    if isequal(path2(end,:),[robotState2.x robotState2.y])
        EndFlag2 = true;
        disp('Goal 2 Reached.')
    end
